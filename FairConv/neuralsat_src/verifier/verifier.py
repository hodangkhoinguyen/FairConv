...
class Verifier:
    ...
    def verify(self: 'Verifier', dnf_objectives: 'DnfObjectives', preconditions: list = [], timeout: int | float = 3600.0, force_split: str | None = None) -> dict:
        ...
        for idx in self.status:
            falsified = round(self.status[idx]["falsified"] * 100, 2)
            proved = round(self.status[idx]["proved"] * 100, 2)
                ...
                while True: 
                    try:
                        ...
                        if is_cuda_out_of_memory(exception):
                            if os.environ.get('NEURALSAT_TRACE'):
                                print(f'[TRACE] OOM at batch={self.batch}, halving and restarting from scratch', flush=True)
                            if self.batch == 1:
    ...
    def _initialize(self: 'Verifier', objective, preconditions: dict, reference_bounds: dict | None) -> DomainsList | list:
        ...
        # credit objectives resolved at depth 0 (no splitting needed) — otherwise this
        # mass is silently dropped and permanently miscounted as "undecided"
        for obj_id in domains_list.init_proved_ids:
            self.result[obj_id]["proved"] += 1
        for obj_id in domains_list.init_falsified_ids:
            self.result[obj_id]["falsified"] += 1
    ...
    def _verify_one(self: 'Verifier', objective, preconditions: dict, reference_bounds: dict | None, timeout: int | float) -> dict:
        ...
        while len(self.domains_list) > 0:
            ...
            if os.environ.get('NEURALSAT_TRACE'):
                ...
                qlen = len(self.domains_list)
                if qlen:
                    min_depth = self.domains_list.split_depth.min().item()
                    max_depth = self.domains_list.split_depth.max().item()
                else:
                    min_depth = max_depth = -1
                print(f'[TRACE] t={elapsed:.1f}s iter={self.iteration} qlen={qlen} batch={self.batch} depth=[{min_depth},{max_depth}] result={self.result}', flush=True)
